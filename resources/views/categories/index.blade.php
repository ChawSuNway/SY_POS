                    ...
                    <div class="table-wrap">
                        <table class="tbl">
                            ...
                            @forelse(($categories[$type] ?? []) as $cat)
                                <tr>
                                    <td>
                                        <input type="text" name="name" value="{{ $cat->name }}" form="cat-edit-{{ $cat->id }}" required>
                                    </td>
                                    <td>
                                        <label class="check">
                                            <input type="checkbox" name="is_active" value="1" form="cat-edit-{{ $cat->id }}" {{ $cat->is_active ? 'checked' : '' }}>
                                        </label>
                                    </td>
                                    <td class="num">
                                        <form id="cat-edit-{{ $cat->id }}" method="POST" action="{{ route('categories.update',$cat) }}" style="display:none">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="type" value="{{ $cat->type }}">
                                        </form>
                                        <div class="btn-row" style="justify-content:flex-end">
                                            <button type="submit" form="cat-edit-{{ $cat->id }}" class="btn primary sm">{{ __('app.save') }}</button>
                                            <form method="POST" action="{{ route('categories.destroy',$cat) }}" onsubmit="return confirm('{{ __('app.confirm_delete') }}')">
                                                @csrf
                                                @method('DELETE')
